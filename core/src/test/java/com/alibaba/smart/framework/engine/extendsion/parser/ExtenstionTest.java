...
public class ExtenstionTest {
    ...
    public void testServiceTask() throws Exception {
        ...
        ProcessDefinitionSource processDefinitionSource =
                repositoryCommandService.deploy("process-def/extend/extend.bpmn20.xml", tenantId);
        ...
        for (ExtensionDecorator extensionDecorator : extensionElements.getExtensionList()) {
            if (extensionDecorator instanceof ProcessField) {
                ...
                if ("inputParams".equals(processField.getName())) {
                    Assert.assertEquals(
                            "[{\"key\":\"buyerOrderMessageCount\",\"type\":\"java.lang.Long\"}]",
                            processField.getValue());
                } else if ("outputParams".equals(processField.getName())) {
                    Assert.assertEquals(
                            "[{\"key\":\"logisticsSpecialEventExtendDay\",\"type\":\"java.lang.Long\"}]",
                            processField.getValue());
                }
            }
        }
        ...
        for (Map.Entry<String, IdBasedElement> entry :
            ...
            if (entry.getValue() instanceof BusinessRuleTask) {
                ...
                Assert.assertNotNull(
                        businessRuleTask.getProperties().get("class"),
                        "com.test.delegate.DecisionRuleTask");
                ...
                for (ExtensionDecorator extensionDecorator : extensionElements.getExtensionList()) {
                    if (extensionDecorator instanceof ProcessField) {
                        ...
                        if ("interval".equals(processField.getName())) {
                            ...
                            Assert.assertEquals(
                                    "[{\"key\":\"logisticsSpecialEventExtendDay\",\"type\":\"java.lang"
                                        + ".Long\"}]",
                                    processField.getValue());
                        }
                    }
