...
public abstract class InstanceUtil {
    ...
    public static List<ExecutionInstance> findActiveExecution(ProcessInstance processInstance) {
        ...
        List<ExecutionInstance> matchedExecutionInstanceList =
                new ArrayList<ExecutionInstance>(activityInstances.size());
        for (ActivityInstance activityInstance : activityInstances) {
            List<ExecutionInstance> executionInstancesOfAI =
    ...
    public static void enrich(Map<String, Object> request, TaskInstance taskInstance) {
        ...
        if (null != request) {
            String comment =
                    ObjectUtil.obj2Str(
                            request.get(RequestMapSpecialKeyConstant.TASK_INSTANCE_COMMENT));
            taskInstance.setComment(comment);

            String extension =
                    ObjectUtil.obj2Str(
                            request.get(RequestMapSpecialKeyConstant.TASK_INSTANCE_EXTENSION));
            taskInstance.setExtension(extension);

            Integer priority =
                    ObjectUtil.obj2Integer(
                            request.get(RequestMapSpecialKeyConstant.TASK_INSTANCE_PRIORITY));
