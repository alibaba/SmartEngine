...
public interface ProcessEngineConfiguration {
    ...
    /**
     * 主要用于外部扩展。 在生产环境下必须设置。 强烈建议该接口返回的是不重复的 id。
     *
     * @param idGenerator
     */
    ...
    /**
     * 主要用于外部扩展。 在生产环境下必须设置。 在系统集成中，SmartEngine 为了和外部系统方便的交互，则必须要提供相应的机制来获取外部服务对象。 该机制主要用于
     * smart:class 机制。
     *
    ...
    /**
     * 主要用于外部扩展。 可选扩展。 设计目的是用来处理各种 Delegation 内部抛出去的异常。
     *
    ...
    /** 一般不需要扩展。 但当你需要针对Delegation执行时抛出的异常，超时等做出精细化处理时，才需要扩展此类。 */
    ...
    /**
     * 主要用于外部扩展。 可选扩展。 设计目的是用来处理任务的处理者。 该扩展主要用于 database模式下的 userTask 场景。
     *
    ...
    /**
     * 目前使用场景较少，主要用于兼容老版本的xml namespace 场景。 一般使用场景下，直接忽略该配置即可
     *
     * @param extension
     */
    ...
    // 是否要干掉 用于配置扩展,默认可以为空。设计目的是根据自己的业务需求,来自定义存储(该机制会绕过引擎自带的各种Storage机制,powerful and a little
    // UnSafe)。。
