...
public class DefaultProcessDefinitionContainer
        implements ProcessDefinitionContainer, ProcessEngineConfigurationAware {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(DefaultProcessDefinitionContainer.class);
    ...
    @Getter
    private final Map<String, PvmProcessDefinition> pvmProcessDefinitionConcurrentHashMap =
            new ConcurrentHashMap<String, PvmProcessDefinition>();

    /** 同上. */
    @Getter
    private final Map<String, ProcessDefinition> processDefinitionConcurrentHashMap =
    ...
    @Override
    public void install(
            PvmProcessDefinition pvmProcessDefinition, ProcessDefinition processDefinition) {
        ...
        String uniqueKey =
                IdAndVersionUtil.buildProcessDefinitionUniqueKey(
    ...
    public void uninstall(String processDefinitionId, String version) {
        String uniqueKey =
                IdAndVersionUtil.buildProcessDefinitionUniqueKey(
                        processDefinitionId, version, null);
    ...
    public void uninstall(String processDefinitionId, String version, String tenantId) {
        if (StringUtil.isEmpty(tenantId)) {
            throw new EngineException(
                    "tenantId can't be null,when uninstall tenant's process definition");
        }
    ...
    public PvmProcessDefinition getPvmProcessDefinition(
        ...
        return this.getPvmProcessDefinition(
    ...
    public boolean isProcessDefinitionShareMode() {
        ConfigurationOption processDefinitionShareOption =
                processEngineConfiguration
                        .getOptionContainer()
                        .get(
                                ProcessDefinitionMultiTenantShareOption
                                        .PROCESS_DEFINITION_MULTI_TENANT_SHARE_OPTION
                                        .getId());
    ...
    public ProcessDefinition getProcessDefinition(String processDefinitionId, String version) {
        return this.processDefinitionConcurrentHashMap.get(
                IdAndVersionUtil.buildProcessDefinitionKey(processDefinitionId, version));
    }
    ...
    private void installPvmProcessDefinition(
            String uniqueKey, PvmProcessDefinition pvmProcessDefinition) {
        pvmProcessDefinition.setIdAndVersion(uniqueKey);

        PvmProcessDefinition existedPvmProcessDefinition =
                pvmProcessDefinitionConcurrentHashMap.get(uniqueKey);
        if (null != existedPvmProcessDefinition) {
            LOGGER.warn(
                    " Duplicated processDefinitionId and version found for unique key "
                            + uniqueKey
                            + " , but it's ok for deploy the process definition repeatedly. BUT"
                            + " this message should be NOTICED. ");
        }
    ...
    private void installProcessDefinition(String uniqueKey, ProcessDefinition processDefinition) {
        ...
        ProcessDefinition existedPvmProcessDefinition =
                processDefinitionConcurrentHashMap.get(uniqueKey);
        if (null != existedPvmProcessDefinition) {
            LOGGER.warn(
                    " Duplicated processDefinitionId and version found for unique key "
                            + uniqueKey
