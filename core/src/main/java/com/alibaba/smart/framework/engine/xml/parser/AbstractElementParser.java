...
public abstract class AbstractElementParser<M extends BaseElement>
        implements ElementParser<M>, ProcessEngineConfigurationAware {
        ...
        if (attributeCount > 0) {
            for (int i = 0; i < attributeCount; i++) {
                ...
                if (null != value && value instanceof BaseElement) {
                    ...
                    LOGGER.debug(
                            attributeQName + " is ignored when parsing attribute from " + model);
                }
            }
        }
    }

    private Object parseSingleAttribute(
            QName attributeName, XMLStreamReader reader, ParseContext context) {

        return xmlParserFacade.parseAttribute(attributeName, reader, context);
    }

    private void parseMultiChildren(M model, XMLStreamReader reader, ParseContext context)
