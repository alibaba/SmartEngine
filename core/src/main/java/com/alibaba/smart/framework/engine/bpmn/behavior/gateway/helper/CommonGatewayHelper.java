...
public abstract class CommonGatewayHelper {
     ...
     * @return
     */
    public static <T extends AbstractGateway> Map<String, String> findMatchedJoinParallelGateway(
            PvmProcessDefinition pvmProcessDefinition, Class<T> clazz) {
        ...
        for (T parallelGateway : elementListByType) {
            PvmActivity pvmActivity =
                    pvmProcessDefinition.getActivities().get(parallelGateway.getId());
    ...
    private static void findOutAllForkJoinPairs(
        ...
        for (Map.Entry<String, PvmTransition> transitionEntry : outcomeTransitions.entrySet()) {
            ...
            if (isForkGateway(mayBeJoinTarget)) {
                ...
                resultMap.put(
                        forkPvmActivity.getModel().getId(), mayBeJoinTarget.getModel().getId());
                break;
            } else {
                // do nothing
            }
    ...
    private static PvmActivity filterNonParallelGateway(
            PvmActivity currentPvmActivity, PvmProcessDefinition pvmProcessDefinition) {
        ...
        for (Map.Entry<String, PvmTransition> transitionEntry : outcomeTransitions.entrySet()) {
            ...
            if (isForkGateway(mayBeJoinTarget) || isJoinGateway(mayBeJoinTarget)) {
                ...
                PvmActivity result =
                        filterNonParallelGateway(mayBeJoinTarget, pvmProcessDefinition);
                if (result != null) {
                    // 如果递归调用找到了目标网关，立即返回
                    return result;
                }
        ...
        throw new EngineException(
                "should find one parallel gateway :" + currentPvmActivity.getModel().getId());
    }

    public static <T extends Activity> List<T> getElementListByType(
            Map<String, PvmActivity> pvmActivityMap, Class<T> elementType) {
    ...
    public static void leaveAndConcurrentlyForkIfNeeded(
            ExecutionContext context, PvmActivity pvmActivity, Collection<PvmTransition> values) {

        int outComeTransitionSize = values.size();

        ExecutorService executorService =
                context.getProcessEngineConfiguration().getExecutorService();
        ...
        if (null == executorService) {
            ...
            // 当子线程执行结束时,看下该分支是否到达了fork对应的join(考虑到嵌套),
            // 如果所有分支都已经完成(注意事项:检查到达该fork对应的join节点,需要注意嵌套,父join找父fork,子join找子join),
            ...
            ParallelGatewayConstant.ExecuteStrategy executeStrategy =
                    getExecuteStrategy(properties);
            boolean isSkipTimeout =
                    isSkipTimeout(
                            (String)
                                    MapUtil.safeGet(
                                            properties,
                                            ParallelGatewayConstant.SKIP_TIMEOUT_EXCEPTION));

            // 注意: 重新赋值 如果能匹配到自定义的线程池，直接使用。 允许扩展并行网关的3种属性: timeout="300" strategy="any"
            // poolName="poolA" skipTimeoutExp="true"  使用方法详见
            // ServiceOrchestrationParallelGatewayTest
            executorService =
                    useSpecifiedExecutorServiceIfNeeded(properties, processEngineConfiguration);

            List<PvmActivityTask> pvmActivityTaskList =
                    new ArrayList<PvmActivityTask>(outComeTransitionSize);
            ...
            try {
                ...
                List<ExecutionContext> subThreadContextList =
                        acquireResults(
                                context,
    ...
    private static void initTaskList(
            ...
            ContextFactory contextFactory,
            Collection<PvmTransition> values,
            List<PvmActivityTask> taskList) {
        for (PvmTransition value : values) {
            ...
            PvmActivityTask pvmActivityTask =
                    context.getProcessEngineConfiguration()
                            .getPvmActivityTaskFactory()
                            .create(target, subThreadContext);

            taskList.add(pvmActivityTask);
        }
    }

    private static List<ExecutionContext> acquireResults(
            ExecutionContext context,
            ProcessEngineConfiguration processEngineConfiguration,
            Long latchWaitTime,
            List<Future<ExecutionContext>> futureExecutionResultList)
        ...
        List<ExecutionContext> subThreadExecutionContextList =
                new ArrayList<>(futureExecutionResultList.size());
        for (Future<ExecutionContext> future : futureExecutionResultList) {
            try {

                if (hasValidLatchWaitTime(latchWaitTime)) {
                    ExecutionContext subThreadExecutionContext =
                            future.get(latchWaitTime, TimeUnit.MILLISECONDS);
    ...
    public static void chooseOnlyOne(PvmActivity pvmActivity, ExecutionContext context) {
        ...
        for (PvmTransition matchedPvmTransition : matchedTransitions) {
            ...
            ListenerExecutor listenerExecutor =
                    context.getProcessEngineConfiguration().getListenerExecutor();
    ...
    public static List<PvmTransition> calcMatchedTransitions(
            PvmActivity pvmActivity, ExecutionContext context) {
        String processDefinitionActivityId = pvmActivity.getModel().getId();
        Map<String, PvmTransition> outcomeTransitions = pvmActivity.getOutcomeTransitions();

        List<PvmTransition> matchedTransitions =
                new ArrayList<PvmTransition>(outcomeTransitions.size());
        ...
        if (0 == matchedTransitions.size()) {
            ...
            if (MapUtil.isNotEmpty(properties)) {
                ...
                throw new EngineException(
                        "properties can not be empty,  check activity id :"
