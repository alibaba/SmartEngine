...
public class ParallelGatewayBehavior extends AbstractActivityBehavior<ParallelGateway> {
    ...
    public boolean enter(ExecutionContext context, PvmActivity pvmActivity) {
        ...
        ConfigurationOption serviceOrchestrationOption =
                processEngineConfiguration
                        .getOptionContainer()
                        .get(ConfigurationOption.SERVICE_ORCHESTRATION_OPTION.getId());
        ...
        if (serviceOrchestrationOption.isEnabled()) {
            ...
            ParallelServiceOrchestration parallelServiceOrchestration =
                    context.getProcessEngineConfiguration().getParallelServiceOrchestration();
    ...
    private boolean processDefaultLogic(
            ExecutionContext context, PvmActivity pvmActivity, ParallelGateway parallelGateway) {
        ...
        if (CommonGatewayHelper.isForkGateway(pvmActivity)) {
            ...
            synchronized (processInstance) {
                ...
                // Merge 数据库中和内存中的EI。如果是 custom模式，则可能会存在重复记录(因为custom也是从内存中查询)，所以这里需要去重。 如果是
                // DataBase 模式，则不会有重复的EI.
                return super.doa(
                        context,
                        parallelGateway,
                        processInstance,
    ...
    private boolean common(
            ...
            List<ExecutionInstance> executionInstanceListFromDB,
        ...
        List<ExecutionInstance> mergedExecutionInstanceList =
