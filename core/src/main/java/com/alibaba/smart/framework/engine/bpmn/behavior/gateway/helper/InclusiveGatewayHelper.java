...
public class InclusiveGatewayHelper {
    ...
    // 添加缓存键生成方法
    private static String generateActivityTreeCacheKey(
            ProcessInstance processInstance,
            PvmActivity forkedPvmActivity,
            PvmActivity joinedPvmActivity) {
        return processInstance.getUniqueProcessDefinitionIdAndVersion()
                + ":"
                + forkedPvmActivity.getModel().getId()
                + ":"
                + joinedPvmActivity.getModel().getId();
    }

    // 添加 JOIN_LATCH_COUNT 缓存键生成方法
    private static String generateJoinLatchCountCacheKey(
            ProcessInstance processInstance,
            ActivityTreeNode node,
            List<String> triggerActivityIds) {
        // 使用流程实例ID、节点ID和触发的活动ID列表作为缓存键
        String processDefinitionIdAndVersion =
    ...
    public static void cacheTriggerActivityIdsToContext(
            PvmActivity pvmActivity,
            ExecutionContext context,
            List<PvmTransition> matchedTransitions) {
        List<String> triggerActivityIds =
                matchedTransitions.stream()
                        .map(pvmTransition -> pvmTransition.getTarget())
                        .map(activity -> activity.getModel().getId())
    ...
    public static void persistMatchedTransitionsEagerly(
            PvmActivity pvmActivity,
            ExecutionContext context,
            List<String> triggerTransitions,
        ...
        variableInstanceStorage.insert(
                variablePersister, variableInstance, processEngineConfiguration);
    }

    public static List<String> calcTriggerActivityIds(
            ExecutionContext context,
            VariableInstanceStorage variableInstanceStorage,
        ...
        String processDefinitionActivityId =
                forkedExecutionInstanceOfInclusiveGateway.getProcessDefinitionActivityId();
        ...
        return findTriggerActivityIdsFromDB(
                variableInstanceStorage,
                forkedExecutionInstanceOfInclusiveGateway,
                variablePersister,
                key,
                processEngineConfiguration);
    }

    private static List<String> findTriggerActivityIdsFromDB(
            VariableInstanceStorage variableInstanceStorage,
            ExecutionInstance forkedExecutionInstanceOfInclusiveGateway,
            VariablePersister variablePersister,
            String key,
        ...
        List<VariableInstance> list =
                variableInstanceStorage.findList(
                        forkedExecutionInstanceOfInclusiveGateway.getProcessInstanceId(),
                        forkedExecutionInstanceOfInclusiveGateway.getInstanceId(),
                        variablePersister,
                        forkedExecutionInstanceOfInclusiveGateway.getTenantId(),
                        processEngineConfiguration);
        Optional<VariableInstance> first =
                list.stream()
                        .filter(variableInstance -> key.equals(variableInstance.getFieldKey()))
                        .findFirst();
        ...
        List<String> triggerActivityIds =
                (List<String>)
                        variablePersister.deserialize(
                                variableInstance.getFieldKey(),
                                variableInstance.getFieldType().getName(),
                                fieldValue);
        return triggerActivityIds;
    }
    ...
    public static ActivityTreeNode buildActivityTreeFromJoinToFork(
            PvmActivity forkedPvmActivity,
            PvmActivity joinedPvmActivity,
            ProcessInstance processInstance) {
        // 生成缓存键
        String cacheKey =
                generateActivityTreeCacheKey(processInstance, forkedPvmActivity, joinedPvmActivity);
    ...
    private static void buildActivityTree(
            Map<String, PvmTransition> transitions,
            String forkGatewayActivityId,
    ...
    public static PvmActivity getForkPvmActivity(
            ProcessInstance processInstance,
            ExecutionInstance forkedExecutionInstanceOfInclusiveGateway,
            ProcessEngineConfiguration processEngineConfiguration) {
        ...
        PvmActivity forkPvmActivity =
                pvmProcessDefinition
                        .getActivities()
                        .get(
                                forkedExecutionInstanceOfInclusiveGateway
                                        .getProcessDefinitionActivityId());
        return forkPvmActivity;
    }

    public static List<ExecutionInstance> calcAllExecutionInstances(
            ExecutionContext context, ExecutionInstanceStorage executionInstanceStorage) {
        ProcessInstance processInstance = context.getProcessInstance();
        ProcessEngineConfiguration processEngineConfiguration =
                context.getProcessEngineConfiguration();

        List<ExecutionInstance> allExecutionInstanceList =
                executionInstanceStorage.findAll(
        ...
        if (CollectionUtil.isEmpty(allExecutionInstanceList)) {
            ...
            allExecutionInstanceList =
                    processInstance.getActivityInstances().stream()
                            .flatMap(
                                    activityInstance ->
                                            activityInstance.getExecutionInstanceList().stream())
                            .collect(Collectors.toList());
        }
        return allExecutionInstanceList;
    }

    public static ExecutionInstance findForkedExecutionInstance(
            ExecutionContext context,
            ExecutionInstance joinedExecutionInstanceOfInclusiveGateway,
        ...
        ExecutionInstance forkedExecutionInstanceOfInclusiveGateway =
                executionInstanceStorage.find(
                        joinedExecutionInstanceOfInclusiveGateway.getBlockId(),
                        joinedExecutionInstanceOfInclusiveGateway.getTenantId(),
                        processEngineConfiguration);

        if (null == forkedExecutionInstanceOfInclusiveGateway) {
            // 说明此时还没落库（SE 目前的设计是一次调用链结束后，才会在最后时刻落库。 这个时候需要从内存中查询）
            ExecutionInstance matchedExecutionInstance =
                    context.getProcessInstance().getActivityInstances().stream()
                            .flatMap(
                                    activityInstance ->
                                            activityInstance.getExecutionInstanceList().stream())
                            .filter(
    ...
    public static int calcCountOfTheJoinLatch(
            ActivityTreeNode node,
            List<String> triggerActivityIds,
            ProcessInstance processInstance) {
        // 生成缓存键
        ...
        for (ActivityTreeNode leaf : triggeredLeafNodes) {
            ...
            if (CollectionUtil.isNotEmpty(ancestorIdSet)) {
                ...
                // 如果没有重叠，则计数+1，并将当前叶子节点的祖先节点ID添加到已计数集合中 (避免因触发同一祖先的两个子节点,而导致重复计数  unbalanced
                // gateway)
                if (!hasOverlap) {
                    count++;
    ...
    private static void onlyCollectAncestors(
            String leafId, ActivityTreeNode node, Set<String> ancestorIds) {
    ...
    private static List<ActivityTreeNode> getTriggeredLeafNodes(
            ActivityTreeNode node, List<String> triggerActivityIds) {
        // 获取所有叶子节点
        ...
        // 过滤出触发的叶子节点
        List<ActivityTreeNode> triggeredLeafNodes =
                allLeafNodes.stream()
