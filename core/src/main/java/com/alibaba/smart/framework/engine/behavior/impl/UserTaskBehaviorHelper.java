...
public class UserTaskBehaviorHelper {
    ...
    static List<TaskAssigneeCandidateInstance> getTaskAssigneeCandidateInstances(
            ExecutionContext context, UserTask userTask) {
        TaskAssigneeDispatcher taskAssigneeDispatcher =
                context.getProcessEngineConfiguration().getTaskAssigneeDispatcher();
     ...
     * @return
     */
    static List<TaskAssigneeCandidateInstance> findBatchOfHighestPriorityTaskAssigneeList(
        ...
        if (CollectionUtil.isNotEmpty(taskAssigneeCandidateInstanceList)) {
            Collections.sort(
                    ...
                    new Comparator<TaskAssigneeCandidateInstance>() {
                        @Override
                        public int compare(
                                TaskAssigneeCandidateInstance one,
                                TaskAssigneeCandidateInstance two) {
                            return two.getPriority() - one.getPriority();
                        }
                    });
        }
        // 优先级低的和所有相同的
        List<TaskAssigneeCandidateInstance> newTaskAssigneeCandidateInstanceList =
                new ArrayList<TaskAssigneeCandidateInstance>(taskAssigneeCandidateInstanceSize);
        int minPriority = 0;
    ...
    static void buildTaskAssigneeInstance(
    ...
    public static void markDoneEIAndCancelTI(
        ...
        for (ExecutionInstance instance : totalExecutionInstanceList) {
            if (instance.isActive()) {
                MarkDoneUtil.markDoneExecutionInstance(
                        instance, executionInstanceStorage, processEngineConfiguration);
            }
        }
        ...
        List<TaskInstance> allTaskInstanceList =
                queryAllTaskInstanceList(
                        executionInstance, processEngineConfiguration, taskInstanceStorage);
    ...
    static void compensateExecutionAndTask(
        ...
        TaskAssigneeStorage taskAssigneeStorage =
                processEngineConfiguration
                        .getAnnotationScanner()
                        .getExtensionPoint(ExtensionConstant.COMMON, TaskAssigneeStorage.class);
        Map<String, List<TaskAssigneeInstance>> taskAssigneeInstanceMap =
                taskAssigneeStorage.findAssigneeOfInstanceList(
                        taskInstanceIdList,
        ...
        if (taskAssigneeMap.size() > 0) {
            List<TaskAssigneeCandidateInstance> newTaskAssigneeList =
                    UserTaskBehaviorHelper.findBatchOfHighestPriorityTaskAssigneeList(
                            new ArrayList<TaskAssigneeCandidateInstance>(taskAssigneeMap.values()));
            for (TaskAssigneeCandidateInstance taskAssigneeCandidateInstance :
                    newTaskAssigneeList) {
                ExecutionInstance newExecutionInstance =
                        executionInstanceFactory.create(activityInstance, context);
                executionInstanceStorage.insert(newExecutionInstance, processEngineConfiguration);

                TaskInstance taskInstance =
                        taskInstanceFactory.create(userTask, newExecutionInstance, context);
    ...
    static List<TaskInstance> queryAllTaskInstanceList(
            ExecutionInstance executionInstance,
            ProcessEngineConfiguration processEngineConfiguration,
            TaskInstanceStorage taskInstanceStorage) {
    ...
    public static void abortAndSetNeedPause(
            ExecutionContext context,
            ExecutionInstance executionInstance,
