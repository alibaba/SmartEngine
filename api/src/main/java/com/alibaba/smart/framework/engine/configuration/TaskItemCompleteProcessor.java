...
public interface TaskItemCompleteProcessor {
    ...
    /**
     * 是否可以驱动到下一个主节点
     * 返回值： key=canDrive(是否可以驱动到下一个节点)  key=tag(如果可以驱动到下一个节点，则当前节点的审核结果的标签值)
     * 例子：Map<String,String> map = new HashMap<>;
     *      map.put("canDrive","true");
     *      map.put("tag","pass");
     */
    Map<String,String> canDriveNextMainElement(String processInstanceId, String taskInstanceId, Activity activity, SmartEngine smartEngine);
}
